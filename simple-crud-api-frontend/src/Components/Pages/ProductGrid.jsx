...
const ProductGrid = ({
  ...
  return (
    <div className="container mx-auto px-4 mt-4">
      {username && (
        <h3 className="text-center mb-4 text-lg md:text-xl">Welcome, {username}!</h3>
      )}
      <h2 className="text-center mb-4 text-2xl md:text-3xl font-semibold">
        Our Products
      </h2>
        ...
        <>
            ...
            {!Array.isArray(products) ? (
              ...
              products.map((product) => (
                <div
                  ...
                  key={product._id}
                  className="group relative bg-white border rounded-lg shadow-md overflow-hidden hover:shadow-lg lg:max-w-[250px] transition-shadow duration-300"
                >
                  ...
                  <div className="p-2 sm:p-4 flex flex-col">
                    <h5 className="text-sm sm:text-lg md:text-xl font-semibold text-gray-900 mb-1 sm:mb-2">
                      {product.name}
                    </h5>
          ...
          <div className="flex justify-center mt-6 space-x-4">
            <button
              ...
              className={`px-4 py-2 text-sm sm:text-base md:text-lg font-medium text-white ${
                currentPage === 1 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
              } rounded-lg transition-colors duration-300`}
            >
              Previous
            </button>
            <span className="px-4 py-2 text-sm sm:text-base md:text-lg font-medium text-gray-700">
              Page {currentPage}
            </span>
            <button
              onClick={nextPage}
              disabled={products.length < 8} // Disable if fewer than 8 products (end of list)
              className={`px-4 py-2 text-sm sm:text-base md:text-lg font-medium text-white ${
                products.length < 8 ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'
              } rounded-lg transition-colors duration-300`}
            >
              Next
            </button>
